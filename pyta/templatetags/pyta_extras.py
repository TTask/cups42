...
class AdminLinkResolver(template.Node):
	...
	def render(self, context):
		try:
			...
			ref = urlresolvers.reverse('admin:%s_%s_change' % ( model._meta.app_label, model.__class__.__name__.lower()),
                                args=(model.id,))
		except:
			ref = ""
		return ref
...
def get_admin_link(parser, token):
	try:
		...
		raise template.TemplateSyntaxError('%r  tag requires a single object from your application models' % token.contents.split()[0])
